<template>
  <div class="HomeContent">
    ...
    <div style="margin: 10px 5px;">
      ...
      <el-upload
        ref="upload"
        :on-success="importClazz"
        style="display: inline-block; margin-left: 5px;margin-right: 5px;"
        action="http://47.96.157.155:9090/admin/clazz/import"
        :headers="headers"
    ...
    >
      ...
      >
        ...
        >
          <el-select
            v-model="newClazzForm.department"
            style="width: 200px;"
            placeholder="请选择院系"
...
export default {
  ...
  methods: {
    async updateStudentTotal() {
      if (this.is_click) {
        this.$message.warning("已经更新过，请5min之后再试");
        return;
      }
      const res = await updateStudentTotal();
      if (res.code === 200) {
        this.is_click = true;
        this.pageNum = 1;
        this.pageSize = 5;
    ...
    async getAllProfessions() {
      const res = await getAllProfessions(this.queryParams, 1e3, 1);
      if (res.code === 200) {
        this.professionList = res.data;
        this.professions = res.data.map((v) => v.profession);
        this.departments = this.unique(res.data, "department").map(
          (v) => v.department
        );
      } else {
        this.$message.error(res.msg);
    ...
    changeDepartment() {
      if (
        !this.newClazzForm.department ||
        this.newClazzForm.department === ""
      ) {
        this.professions = this.professionList.map((v) => v.profession);
      } else {
        this.professions = this.professionList
          .filter((v) => v.department === this.newClazzForm.department)
          .map((v) => v.profession);
      }
    },
    importClazz(response, file, fileList) {
